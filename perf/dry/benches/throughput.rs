...
fn setup_engine() -> (Engine, SharedMemHandle, SmallRng) {
...
fn bench_fuzzer_throughput(c: &mut Criterion) {
    ...
    let (mut engine_mut, _, _) = setup_engine();
    group.bench_function("mutate_only", |b| {
        b.iter(|| {
    ...
    let (mut engine_full, shmem_queue, mut rng) = setup_engine();
    let mut epoch: u64 = 0;

    group.bench_function("engine_full", |b| {
        b.iter(|| {
